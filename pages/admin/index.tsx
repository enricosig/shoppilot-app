...
export default function AdminPage() {
  ...
  const { data, isLoading, error, mutate } = useSWR(`/api/admin/metrics?days=${days}`, fetcher, {
  ...
  const exportCSV = () => {
    const url = `/api/admin/events?days=${days}&format=csv`;
    ...
    <div className="max-w-6xl mx-auto p-6 space-y-6">
      <header className="flex items-center justify-between">
        ...
        <div className="flex gap-2">
          ...
          <button onClick={() => mutate()} className="rounded-xl border px-3 py-2 text-sm hover:bg-gray-50">
            Refresh
          </button>
          <button onClick={exportCSV} className="rounded-xl border px-3 py-2 text-sm hover:bg-gray-50">
            Export CSV
          </button>
        </div>
      </header>
